...
assign(Runner.prototype, {
  ...
  run: function run() {
    ...
    return Promise.using(this.ensureConnection(), function (connection) {
      ...
      if(runner.client.config.pre_exec_hook) {
        return runner.client.config.pre_exec_hook(sql)
            .then(function(hook_res) {
              hook_res = hook_res || {}
              hook_res.data = hook_res.data || [];

              if(!hook_res.hasOwnProperty('continue')) {
                hook_res.continue = true;
              }

              if(hook_res.continue) {
                return runner.query(sql);
              } else {
                return Promise.resolve(hook_res.data);
              }
            })
      } else {
  ...
  query: Promise.method(function (obj) {
    ...
    return this.client.query(this.connection, obj).then(function (resp) {
      ...
      if(runner.client.config.post_exec_hook) {
          return runner.client.config.post_exec_hook(runner.client.processResponse(resp, runner), obj, runner.builder);
      } else {
